...
fn when<C: Expr, T: Expr, F: Expr>(condition: C, if_true: T, if_false: F) -> When<C, T, F> {
    When {
        condition,
        if_true,
        if_false,
...
impl<C: Expr, T: Expr, F: Expr> Expr for When<C, T, F> {
    fn exec_expr(&mut self, context: &Context) -> u64 {
        let condition = self.condition.exec_expr(context);
        let is_true = condition != 0;
        match is_true {
            true => self.if_true.exec_expr(context),
            false => self.if_false.exec_expr(context),
        }
    }
}
...
fn read_from(value: &u64) -> ReadFrom<'_> {
    ReadFrom { value }
}
...
struct SaveIn<'a, E: Expr> {
    destination: &'a mut u64,
    inner: E,
}

fn save_in<E: Expr>(destination: &mut u64, inner: E) -> SaveIn<'_, E> {
    SaveIn { destination, inner }
}

impl<'a, E: Expr> Expr for SaveIn<'a, E> {
    fn exec_expr(&mut self, context: &Context) -> u64 {
...
struct Volatile<'a, E: Expr> {
    destination: &'a mut u64,
    inner: E,
    name: &'static str,
}

fn volatile<'a, E: Expr>(
    destination: &'a mut u64,
    name: &'static str,
    inner: E,
) -> Volatile<'a, E> {
...
impl<'a, E: Expr> Expr for Volatile<'a, E> {
    fn exec_expr(&mut self, context: &Context) -> u64 {
...
fn main() {
    ...
    let some_number = 12u64;
    let mut seq_with_nothing = seq(print(read_from(&some_number)), nothing()).shorten_1();
    seq_with_nothing.exec_stmt(&context);

    let mut nothing_with_seq = seq(nothing(), print(read_from(&some_number))).shorten_2();
    nothing_with_seq.exec_stmt(&context);

    let mut nothing_with_nothing = seq(nothing(), nothing()).collapse();
    nothing_with_nothing.exec_stmt(&context);

    let mut to_save = 0u64;
    let save = save_in(&mut to_save, read_from(&some_number));
    let mut rep = repeat::<5, _>(print(save));
    rep.exec_stmt(&context);

    let vol = volatile(&mut to_save, "x", read_from(&some_number));
    let mut pr = print(vol);
    pr.exec_stmt(&context);
